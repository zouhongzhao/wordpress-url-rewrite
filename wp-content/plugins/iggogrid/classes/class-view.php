<?php
/**
 * IggoGrid Base View with members and methods for all views
 *
 * @package IggoGrid
 * @subpackage Views
 * @author Iggo
...
/**
 * IggoGrid Base View class
 * @package IggoGrid
 * @subpackage Views
 * @author Iggo
 * @since 1.0.0
 */
abstract class IggoGrid_View {
...
// 			'id' => 'iggogrid-help', // This should be unique for the screen.
// 			'title' => __( 'IggoGrid Help', 'iggogrid' ),
// 			'content' => '<p>' . $this->help_tab_content() . '</p>'
// 						. '<p>' . sprintf( __( 'More information about IggoGrid can be found on the <a href="%1$s">plugin&#8217;s website</a> or on its page in the <a href="%s">WordPress Plugin Directory</a>.', 'iggogrid' ), 'http://iggogrid.org/', 'https://wordpress.org/plugins/iggogrid/' ) . ' '
// 						. sprintf( __( 'For technical information, please see the <a href="%s">documentation</a>.', 'iggogrid' ), 'http://iggogrid.org/documentation/' ) . ' '
// 						. sprintf( __( '<a href="%1$s">Support</a> is provided through the <a href="%2$s">WordPress Support Forums</a>.', 'iggogrid' ), 'http://iggogrid.org/support/', 'https://wordpress.org/tags/iggogrid' ) . ' '
// 						. sprintf( __( 'Before asking for support, please carefully read the <a href="%s">Frequently Asked Questions</a>, where you will find answers to the most common questions, and search through the forums.', 'iggogrid' ), 'http://iggogrid.org/faq/' ) . '<br />'
// 						. sprintf( __( 'If you like the plugin, <a href="%1$s"><strong>a donation</strong></a> is recommended.', 'iggogrid' ), 'http://iggogrid.org/donate/' ) . '</p>',
// 		) );
// 		// "sidebar" in the help tab
// 		$screen->set_help_sidebar( '<p><strong>' . __( 'For more information:', 'iggogrid' ) . '</strong></p><p><a href="http://iggogrid.org/" target="_blank">iggogrid Website</a></p><p><a href="http://iggogrid.org/faq/" target="_blank">iggogrid FAQ</a></p><p><a href="http://iggogrid.org/documentation/" target="_blank">iggogrid Documentation</a></p><p><a href="http://iggogrid.org/support/" target="_blank">iggogrid Support</a></p>' );
	}
	...
	public function setup( $action, array $data ) {
		...
		if ( is_rtl() ) {
			$this->admin_page->enqueue_style( 'common-rtl', array( 'iggogrid-common' ) );
		}
		$this->admin_page->enqueue_script( 'common', array( 'jquery', 'postbox' ), array(
			'common' => array(
				'ays_delete_single_table' => _n( 'Do you really want to delete this table?', 'Do you really want to delete these tables?', 1, 'iggogrid' ),
				'ays_delete_multiple_tables' => _n( 'Do you really want to delete this table?', 'Do you really want to delete these tables?', 2, 'iggogrid' ),
			)
		) );

		$this->admin_page->add_admin_footer_text();

		// Initialize WP feature pointers for iggogrid
	...
	protected function add_text_box( $id, $callback, $context = 'normal', $wrap = false ) {
		...
		$long_id = "iggogrid_{$this->action}-{$id}";
	...
	protected function add_meta_box( $id, $title, $callback, $context = 'normal', $priority = 'default', $callback_args = null ) {
		...
		add_meta_box( "iggogrid_{$this->action}-{$id}", $title, $callback, null, $context, $priority, $callback_args );
	}
	...
	protected function action_nonce_field( array $data, array $box ) {
		wp_nonce_field( iggogrid::nonce( $this->action ) ); echo "\n";
	}
	...
	protected function action_field( array $data, array $box ) {
		echo "<input type=\"hidden\" name=\"action\" value=\"iggogrid_{$this->action}\" />\n";
	}
	...
	protected function print_nav_tab_menu() {
		?>
		<h2 id="iggogrid-nav" class="nav-tab-wrapper">
			<?php
			echo __( 'iggogrid', 'iggogrid' ) . '<span class="separator"></span>';
			foreach ( $this->data['view_actions'] as $action => $entry ) {
				...
				$url = esc_url( iggogrid::url( array( 'action' => $action ) ) );
	...
	protected function textbox_submit_button( array $data, array $box ) {
		$caption = isset( $data['submit_button_caption'] ) ? $data['submit_button_caption'] : __( 'Save Changes', 'iggogrid' );
		?>
	...
	/**
	 * Initialize the WP feature pointers for iggogrid
	 *
	 * @since 1.0.0
	 */
