...
class IggoGrid_CSS {
	...
	function sanitize_css( $css ) {
		$csstidy = IggoGrid::load_class( 'csstidy', 'class.csstidy.php', 'libraries/csstidy' );
		...
		$csstidy->set_cfg( 'template', dirname( IGGOGRID__FILE__ ) . '/libraries/csstidy/iggogrid-standard.tpl' );
	 ...
	 * @since 1.0.0
	 *
	 * @param string $type "normal" version, "minified" version, or "combined" (with IggoGrid Default CSS) version
	...
	*/
	public function get_custom_css_location( $type, $location ) {
		switch ( $type ) {
			case 'combined':
				$file = 'iggogrid-combined.min.css';
				break;
			case 'minified':
				$file = 'iggogrid-custom.min.css';
				break;
			case 'normal':
			default:
				$file = 'iggogrid-custom.css';
				break;
		}
		...
		switch ( $location ) {
			case 'url':
				 ...
				 */
				$url = apply_filters( 'iggogrid_custom_css_url', $url, $file, $type );
				return $url;
				break;
			case 'path':
				 ...
				 * @param string $type Type of the "Custom CSS" file ("normal", "minified", or "combined").
				 */
				$path = apply_filters( 'iggogrid_custom_css_file_name', $path, $file, $type );
				return $path;
				break;
		}
	}
	...
	/**
	 * Load the contents of the file with the IggoGrid Default CSS
	 *
	 * @since 1.1.0
	 *
	 * @return string|bool IggoGrid Default CSS on success, false on error
	 */
	public function load_default_css_from_file() {
		$filename = IGGOGRID_ABSPATH . 'css/default.min.css';
		// Check if file name is valid (0 means yes)
		if ( 0 !== validate_file( $filename ) ) {
			return false;
		}
	...
	public function save_custom_css_to_file( $custom_css_normal, $custom_css_minified ) {
		 ...
		 */
		if ( ! apply_filters( 'iggogrid_save_custom_css_to_file', true ) ) {
			return false;
		}
	...
	protected function _custom_css_save_helper( $custom_css_normal, $custom_css_minified ) {
		...
		if ( false === $default_css ) {
			...
			$absolute_path = plugins_url( 'css/iggogrid.', IGGOGRID__FILE__ );
			...
			$default_css = str_replace( 'url(iggogrid.', 'url(' . $absolute_path, $default_css );
		}
		$file_content = array(
