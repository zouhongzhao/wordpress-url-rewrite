...
class IggoGrid_Render {
	...
	public function __construct() {
		$this->evalmath = IggoGrid::load_class( 'EvalMath', 'evalmath.class.php', 'libraries', true ); // true for some default constants
	...
	public function set_input( array $table, array $render_options ) {
		 ...
		 */
		$this->table = apply_filters( 'iggogrid_table_raw_render_data', $this->table, $this->render_options );
	}
			...
			foreach ( $elements as $element ) {
				...
				foreach ( $this->render_options["{$action}_{$element}"] as $key => $value ) {
					...
					if ( false !== $range_dash ) {
						...
						if ( ! is_numeric( $start ) ) {
							$start = IggoGrid::letter_to_number( $start );
						}
						$end = substr( $value, $range_dash + 1 );
						if ( ! is_numeric( $end ) ) {
							$end = IggoGrid::letter_to_number( $end );
						}
				...
				foreach ( $this->render_options["{$action}_{$element}"] as $key => $value ) {
					if ( ! is_numeric( $value ) ) {
						$value = IggoGrid::letter_to_number( $value );
					}
	...
	//	$hidden_columns = array_merge( array_diff( $hidden_columns, $this->render_options['show_columns'] ) );
		...
		//SELECT TABLE
		$columns = $this->table['visibility']['columns'];
		...
		//$dbResult[0] =  $this->table['visibility']['columns'];
		if($flag){
			$where = '';
			if($table_name == 'iggogrid_item_info'){
				global $wp;
				$allowGrid = array(
				...
				if(!isset($allowGrid[$wp->request])){
					return;
				}
// 				$typesSql = "SELECT code FROM iggogrid_types where type = '{$allowGrid[$wp->request]}' ";
// 				//  echo $typesSql;die;
// 				$typesArray = array();
...
// 				$typeString = "('".$typeString."')";
				...
				//$dbResult[0] = array('type_code','type_type','images','url_key','destination_name','headline','area','type','person','rooms');
				$customWhere = '';
				if($allowGrid[$wp->request] == 'accommodation'){
					$customWhere .= "and info.price > 0";
				}
// 				$where = ' where info.lang_id = '.$langId.' and info.type_code in '.$typeString.' '.$customWhere.' order by info.destination_name asc';
// 				$sql = "SELECT info.item_id,info.acc_properties,info.images,info.url_key,info.destination_name,info.headline,info.area,info.type,info.person,info.rooms,info.price,
// 				info.product_name,info.price_{$currentSymbol}
// 				FROM iggogrid_item_info as info  $where";
				$typesArray = IggoGrid::getTypesAndAreaByModel($allowGrid[$wp->request],$langId);
				$typeString = implode('","', $typesArray['typeData']);
				if($allowGrid[$wp->request] == 'activity'){
...
// 				$where = ' where info.lang_id = '.$langId.' '.$customWhere.' order by info.destination_name asc';
				//$where = ' where info.lang_id = 1 and types.type = "'.$allowGrid[$wp->request].'" order by info.destination_name asc';
				...
				$dbResult = $typesArray;
				$dbResult['model'] = $allowGrid[$wp->request];
...
// 				echo $sql;
			...
			foreach( $wpdb->get_results($sql) as $key => $row) {
				...
				if($table_name == 'iggogrid_item_info' && $allowGrid[$wp->request] == 'activity' && $row['admin_added'] == 0 && $row['price'] <= 0){
					continue;
				}
				$dbResult['items'][] = $row;//array_values($row);
			}
		}
		$this->table['data'] = $dbResult;
...
// 		die;
		 ...
		 * @param array $render_options The render options for the table.
		 */
		$this->table = apply_filters( 'iggogrid_table_render_data', $this->table, $orig_table, $this->render_options );
	}
	...
	protected function _evaluate_table_data() {
		 ...
		 * @param array $render_options The render options for the table.
		 */
		$this->table = apply_filters( 'iggogrid_table_evaluate_data', $this->table, $orig_table, $this->render_options );
	}
		...
		foreach ( $expressions as $expression ) {
			...
			if ( preg_match_all( '#([A-Z]+)([0-9]+):([A-Z]+)([0-9]+)#', $expression, $referenced_cell_ranges, PREG_SET_ORDER ) ) {
				foreach ( $referenced_cell_ranges as $cell_range ) {
					...
					$first_col = IggoGrid::letter_to_number( $cell_range[1] );
					$first_row = $cell_range[2];
					$last_col = IggoGrid::letter_to_number( $cell_range[3] );
					...
					for ( $col = $col_start; $col < $col_end; $col++ ) {
						for ( $row = $row_start; $row < $row_end; $row++ ) {
							$column = IggoGrid::number_to_letter( $col );
			...
			if ( preg_match_all( '#([A-Z]+)([0-9]+)#', $expression, $referenced_cells, PREG_SET_ORDER ) ) {
				foreach ( $referenced_cells as $cell_reference ) {
					...
					$ref_col = IggoGrid::letter_to_number( $cell_reference[1] ) - 1;
...
// 		$gridlistdata = $this->table['data'][0];
// 		unset($this->table['data'][0]);
// 		foreach ($this->table['data'] as $key=>$data){
// 			$this->table['data'][$key] = array_combine($gridlistdata,$data);
// 		}
		//$dataArray = 
		$GLOBALS['iggoGridListData'] = $this->table['data'];
		
		return IggoGrid::locate_plugin_template('page-outsidecontent.php',true);
		$num_rows = count( $this->table['data'] );
		$num_columns = ( $num_rows > 0 ) ? count( $this->table['data'][0] ) : 0;
		...
		if ( 0 === $num_rows || 0 === $num_columns ) {
			$this->output = sprintf( __( '<!-- The table with the ID %s is empty! -->', 'iggogrid' ), $this->table['id'] );
			return;
		}
		 ...
		 */
		$this->span_trigger = apply_filters( 'iggogrid_span_trigger_keywords', $this->span_trigger, $this->table['id'] );
		...
		if ( $this->render_options['print_name'] ) {
			 ...
			 * @param string $table_id The current table ID.
			 */
			$print_name_html_tag = apply_filters( 'iggogrid_print_name_html_tag', 'h2', $this->table['id'] );
			/**
			 ...
			 * @param string $table_id The current table ID.
			 */
			$print_name_css_class = apply_filters( 'iggogrid_print_name_css_class', "iggogrid-table-name iggogrid-table-name-id-{$this->table['id']}", $this->table['id'] );
		...
		if ( $this->render_options['print_description'] ) {
			 ...
			 * @param string $table_id The current table ID.
			 */
			$print_description_html_tag = apply_filters( 'iggogrid_print_description_html_tag', 'span', $this->table['id'] );
			/**
			 ...
			 * @param string $table_id The current table ID.
			 */
			$print_description_css_class = apply_filters( 'iggogrid_print_description_css_class', "iggogrid-table-description iggogrid-table-description-id-{$this->table['id']}", $this->table['id'] );
		...
		if ( ! $this->render_options['convert_line_breaks'] ) {
			add_filter( 'iggogrid_apply_nl2br', '__return_false', 9 ); // priority 9, so that this filter can easily be overwritten at the default priority
		}
		...
		if ( ! $this->render_options['convert_line_breaks'] ) {
			remove_filter( 'iggogrid_apply_nl2br', '__return_false', 9 ); // priority 9, so that this filter can easily be overwritten at the default priority
		}
		 ...
		 */
		$caption = apply_filters( 'iggogrid_print_caption_text', '', $this->table );
		...
		if ( ! empty( $caption ) ) {
			 ...
			 * @param string $table_id The current table ID.
			 */
			$caption_class = apply_filters( 'iggogrid_print_caption_class', "iggogrid-table-caption iggogrid-table-caption-id-{$this->table['id']}", $this->table['id'] );
		 ...
		 * @param string $table_id The current table ID.
		 */
		if ( apply_filters( 'iggogrid_print_colgroup_tag', false, $this->table['id'] ) ) {
			for ( $col_idx = 0; $col_idx < $num_columns; $col_idx++ ) {
				 ...
				 */
				$attributes = apply_filters( 'iggogrid_colgroup_tag_attributes', $attributes, $this->table['id'], $col_idx + 1 );
		...
		// "class" attribute
		$css_classes = array( 'iggogrid', "iggogrid-id-{$this->table['id']}", $this->render_options['extra_css_classes'] );
		/**
		 ...
		 * @param string $table_id    The current table ID.
		 */
		$css_classes = apply_filters( 'iggogrid_table_css_classes', $css_classes, $this->table['id'] );
		 ...
		 * @param array  $table   The current table.
		 */
		$summary = apply_filters( 'iggogrid_print_summary_attr', $summary, $this->table );
		if ( ! empty( $summary ) ) {
		 ...
		 * @param array $render_options   The render options for the table.
		 */
		$table_attributes = apply_filters( 'iggogrid_table_tag_attributes', $table_attributes, $this->table, $this->render_options );
		...
		$customJs = '<script type="text/javascript">
				jQuery( document ).ready(function() {
					jQuery(document).on("click",".iggogrid tbody tr",function(){
		 ...
		 * @param array  $render_options The render options for the table.
		 */
		$this->output = apply_filters( 'iggogrid_table_output', $output, $this->table, $this->render_options );
	}
	...
	protected function _render_row( $row_idx, $tag ) {
		...
		for ( $col_idx = $this->last_column_idx; $col_idx >= 0; $col_idx-- ) {
			 ...
			 */
			$cell_content = apply_filters( 'iggogrid_cell_content', $cell_content, $this->table['id'], $row_idx + 1, $col_idx + 1 );
			 ...
			 */
			$cell_class = apply_filters( 'iggogrid_cell_css_class', $cell_class, $this->table['id'], $cell_content, $row_idx + 1, $col_idx + 1, $this->colspan[ $row_idx ], $this->rowspan[ $col_idx ] );
			if ( ! empty( $cell_class ) ) {
			 ...
			 * @param int    $rowspan_col    The number of combined rows for this cell.
			 */
			$tag_attributes = apply_filters( 'iggogrid_cell_tag_attributes', $tag_attributes, $this->table['id'], $cell_content, $row_idx + 1, $col_idx + 1, $this->colspan[ $row_idx ], $this->rowspan[ $col_idx ] );
		 ...
		 */
		$row_classes = apply_filters( 'iggogrid_row_css_class', $row_classes, $this->table['id'], $row_cells, $row_idx + 1, $this->table['data'][ $row_idx ] );
		 ...
		 * @param array  $row_data      The content of the cells of the row.
		 */
		$tr_attributes = apply_filters( 'iggogrid_row_tag_attributes', $tr_attributes, $this->table['id'], $row_idx + 1, $this->table['data'][ $row_idx ] );
		...
		if($tag == 'td'){
			...
			$url = "title='{$url}'  style='cursor: pointer;'";
		}
		$row_cells = array_reverse( $row_cells ); // because we looped through the cells in reverse order
		//var_dump($this->table['id']);
		return "<tr{$tr_attributes}   $url type='iggo_table_show_tr'   table_id='".$this->table['id']."'>\n\t" . implode( '', $row_cells ) . "\n</tr>\n";
	}
	...
	protected function safe_output( $string ) {
		 ...
		 * @param string $table_id The current table ID.
		 */
		if ( apply_filters( 'iggogrid_apply_nl2br', true, $this->table['id'] ) ) {
			$string = nl2br( $string );
		}
		return $string;
	}
...
.iggogrid tbody tr td,
.iggogrid tfoot tr th {
	border-top: 1px solid #dddddd;
}
.iggogrid tbody tr:first-child td {
	border-top: 0;
}
.iggogrid thead tr th {
...
.iggogrid tbody tr.odd td {
	background-color: #f9f9f9;
}
.iggogrid tbody tr.even td {
	background-color: #ffffff;
}
.iggogrid .row-hover tr:hover td {
