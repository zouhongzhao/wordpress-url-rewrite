 ...
 * @since 1.0.0
 */
class IggoGrid_Table_Model extends IggoGrid_Model {
	 ...
	 * @var string
	 */
	protected $table_options_field_name = '_iggogrid_table_options';
	 ...
	 * @var string
	 */
	protected $table_visibility_field_name = '_iggogrid_table_visibility';
	...
	public function __construct() {
		parent::__construct();
		$this->model_post = IggoGrid::load_model( 'post' );

		$params = array(
			'option_name' => 'iggogrid_tables',
			'default_value' => $this->default_tables,
		);
		$this->tables = IggoGrid::load_class( 'IggoGrid_WP_Option', 'class-wp_option.php', 'classes', $params );
	}
...
//			'created' => $post->post_date,
	...
	public function load_all( $prime_meta_cache = true, $run_filter = true ) {
		...
		if ( $run_filter ) {
			 ...
			 */
			$table_ids = apply_filters( 'iggogrid_load_all_tables', $table_ids );
		}
	...
	public function add( array $table, $copied_table_id = false ) {
		...
		if ( false !== $copied_table_id ) {
			 ...
			 */
			do_action( 'iggogrid_event_copied_table', $table_id, $copied_table_id );
		} else {
			/**
			 ...
			 */
			do_action( 'iggogrid_event_added_table', $table_id );
		}
	...
	public function copy( $table_id ) {
		...
		if ( '' == trim( $table['name'] ) ) {
			$table['name'] = __( '(no name)', 'iggogrid' );
		}
		$table['name'] = sprintf( __( 'Copy of %s', 'iggogrid' ), $table['name'] );
	...
	public function delete( $table_id ) {
		 ...
		 */
		do_action( 'iggogrid_event_deleted_table', $table_id );
	...
	public function delete_all() {
		 ...
		 * @since 1.1.0
		 */
		do_action( 'iggogrid_event_deleted_all_tables' );
	}
	...
	public function _flush_caching_plugins_caches() {
		 ...
		 */
		if ( ! apply_filters( 'iggogrid_flush_caching_plugins_caches', true ) ) {
			return;
		}
	...
	public function change_table_id( $old_id, $new_id ) {
		 ...
		 */
		do_action( 'iggogrid_event_changed_table_id', $new_id, $old_id );
	 ...
	 *
	 * Important: This scheme is versioned via IggoGrid::table_scheme_version; changes likely need a version update!
	 *
	 * @since 1.0.0
	 *
...
//			'created' => current_time( 'mysql' ),
		 ...
		 */
		return apply_filters( 'iggogrid_table_template', $table );
	}
	...
	// 			$new_table['number']['columns'] = intval( $new_table['number']['columns'] );
			...
			if ( $table_size_check ) {
				...
				$columns = wp_unslash( $new_table['columns'] );
				$num_rows = count($columns);
				$table['new_id'] = isset( $new_table['new_id'] ) ? $new_table['new_id'] : $table['id'];
				$table['name'] = $new_table['name'];
				$table['description'] = $new_table['description'];
				$table['data'] = array_fill( 0, $num_rows, array_fill( 0, $num_rows, '' ) );
				$table['visibility']['rows'] = array_fill( 0, $num_rows, 1 );
				$table['visibility']['columns'] = $columns;
		}
		return $table;
	}
	...
	/**
	 * Merge changes made for IggoGrid 0.6-beta:
	...
	/**
	 * Merge changes made for IggoGrid 0.8-beta:
	 ...
	 * For IggoGrid 0.9-RC and onwards.
	 *
	 * @since 0.9-RC
	 */
	...
	/**
	 * Invalidate all table output caches, e.g. after a plugin update
	 * For IggoGrid pre-0.9-RC updates
	 *
	 * @since 0.9-RC
	 */
