 ...
 * @since 1.0.0
 */
class IggoGrid_Post_Model extends IggoGrid_Model {
	 ...
	 * @var string
	 */
	protected $post_type = 'iggogrid_table';
	...
	protected function _register_post_type() {
		/**
		 * Filter the "Custom Post Type" that IggoGrid uses for storing tables in the database.
		 *
		 * @since 1.0.0
		 *
		 * @param string $post_type The "Custom Post Type" that IggoGrid uses.
		 */
		$this->post_type = apply_filters( 'iggogrid_post_type', $this->post_type );
		$post_type_args = array(
			'labels' => array(
				'name' => 'IggoGrid Tables',
			),
			'public' => false,
			'show_ui' => false,
			'query_var' => false,
			'rewrite' => false,
			'capability_type' => 'iggogrid_table', // this ensures, that WP's regular CPT UI respects our capabilities
			'map_meta_cap' => false, // integrated WP mapping does not fit our needs, therefore use our own in a filter
			'supports' => array( 'title', 'editor', 'excerpt', 'revisions' ),
			'can_export' => true,
		);
		/**
		 * Filter the arguments for the registration of the "Custom Post Type" that IggoGrid uses.
		 *
		 * @since 1.0.0
		 *
		 * @param array $post_type_args Arguments for the registration of the IggoGrid "Custom Post Type".
		 */
		$post_type_args = apply_filters( 'iggogrid_post_type_args', $post_type_args );
