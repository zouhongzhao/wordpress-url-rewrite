...
/* global prompt, confirm, iggogrid_common, iggogrid_list, tb_show, ajaxurl */
...
jQuery( document ).ready( function( $ ) {
	 ...
	 * @since 1.0.0
	 */
	$( '.iggogrid-all-tables' ).on( 'click', '.shortcode a', function( /* event */ ) {
		prompt( iggogrid_list.shortcode_popup, $(this).attr( 'title' ) );
		return false;
	} );
	 ...
	 * @since 1.0.0
	 */
	$( '.iggogrid-all-tables' ).on( 'click', '.table-preview a', function( /* event */ ) {
	 ...
	 * @since 1.0.0
	 */
	$( '#iggogrid-page' ).on( 'click', '.ajax-link', function( /* event */ ) {
		var $link = $(this),
				...
				switch ( action ) {
					case 'hide_message':
						...
						if ( 'donation_nag' === item && '' !== target ) {
							$link.closest( 'div' ).after( '<div class="donation-message-after-click-message updated"><p><strong>' + iggogrid_list['donation-message-' + target] + '</strong></p></div>' );
	...
	$( '#doaction, #doaction2' ).on( 'click', function() {
		var bulk_action,
			confirm_message,
			num_selected = $( '.iggogrid-all-tables' ).find( 'tbody' ).find( 'input:checked' ).length;
		...
		if ( 'delete' === $( '#bulk-action-' + bulk_action ).val() ) {
			if ( 1 === num_selected ) {
				confirm_message = iggogrid_common.ays_delete_single_table;
			} else {
				confirm_message = iggogrid_common.ays_delete_multiple_tables;
			}
