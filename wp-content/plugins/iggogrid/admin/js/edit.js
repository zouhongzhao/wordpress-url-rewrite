...
/* global alert, confirm, tp, iggogrid_strings, iggogrid_options, ajaxurl, QTags, wpLink, tb_show, tb_remove */
...
jQuery( document ).ready( function( $ ) {
	...
	/**
	 * IggoGrid object, mostly with functionality for the "Edit" screen
	 *
	 * @since 1.0.0
	 */
	...
	tp.table = {
		...
		head_cell: '<th class="head"><span class="sort-control sort-desc" title="' + iggogrid_strings.sort_desc + '"><span class="sorting-indicator"></span></span><span class="sort-control sort-asc" title="' + iggogrid_strings.sort_asc + '"><span class="sorting-indicator"></span></span><span class="move-handle"></span></th>',
		...
		unset_table_changed: function() {
			...
			// @TODO: maybe use .iggogrid-postbox-table here and further below
			$( '#iggogrid_edit-table-information, #iggogrid_edit-table-options, #iggogrid_edit-datatables-features' ).one( 'change', 'input, textarea, select', tp.table.set_table_changed );
		},
		change_id: function( /* event */ ) {
			// empty table IDs are not allowed
			if ( '' === $.trim( $id( 'table-new-id' ).val() ) ) {
				alert( iggogrid_strings.table_id_not_empty );
				$id( 'table-new-id' ).val( tp.table.new_id ).focus().select();
				return;
			}
			// the '0' table ID is not allowed
			if ( '0' === $.trim( $id( 'table-new-id' ).val() ) ) {
				alert( iggogrid_strings.table_id_not_zero );
				$id( 'table-new-id' ).val( tp.table.new_id ).focus().select();
				return;
			}
			...
			if ( confirm( iggogrid_strings.ays_change_table_id ) ) {
				tp.table.new_id = this.value;
				$( '.table-shortcode' ).val( '[' + iggogrid_options.shortcode + ' id=' + tp.table.new_id + ' /]' ).click(); // click() to focus and select
		...
		prepare_ajax_request: function( wp_action, wp_nonce ) {
			...
			return {
				...
				iggogrid: {
					...
					show_table_rows: $(".current_table_rows_show").val(),
					current_columns: $(".current_table_show").val(),
					columns: $(".iggo_grid_edit_columns").val(),
					number: table_number,
					data: table_data,
		...
		preview: {
			trigger: function( /* event */ ) {
				...
				if ( $id( 'option-datatables-paginate' ).prop( 'checked' ) && ! ( /^[1-9][0-9]{0,4}$/ ).test( $id( 'option-datatables-paginate_entries' ).val() ) ) {
					alert( iggogrid_strings.num_pagination_entries_invalid );
					$id( 'option-datatables-paginate_entries' ).focus().select();
					return;
				}
				if ( ( /[^A-Za-z0-9- _]/ ).test( $id( 'option-extra-css-classes' ).val() ) ) {
					alert( iggogrid_strings.extra_css_classes_invalid );
					$id( 'option-extra-css-classes' ).focus().select();
					return;
				}

				$(this).closest( 'p' ).append( '<span class="animation-preview spinner" title="' + iggogrid_strings.preparing_preview + '"/>' );
				$( 'body' ).addClass( 'wait' );
				$id( 'table-preview' ).empty(); // clear preview

				$.post(
						ajaxurl,
						tp.table.prepare_ajax_request( 'iggogrid_preview_table', '#nonce-preview-table' ),
			...
			error: function( message ) {
				$( '.animation-preview' ).closest( 'p' )
					.after( '<div class="ajax-alert preview-error error"><p>' + iggogrid_strings.preview_error + ': ' + message + '</p></div>' );
				$( '.animation-preview' ).remove();
	...
	tp.rows = {
		...
		append: function( /* event */ ) {
			...
			if ( ! ( /^[1-9][0-9]{0,4}$/ ).test( num_rows ) ) {
				alert( iggogrid_strings.append_num_rows_invalid );
		...
		insert: function( event ) {
			...
			if ( 0 === $selected_rows.length ) {
				alert( iggogrid_strings.no_rows_selected );
				return;
			}
		...
		remove: function( /* event */ ) {
			...
			if ( tp.table.rows === $selected_rows.length ) {
				alert( iggogrid_strings.no_remove_all_rows );
				return;
			}

			if ( 1 === $selected_rows.length ) {
				confirm_message = iggogrid_strings.ays_remove_rows_singular;
			} else {
				confirm_message = iggogrid_strings.ays_remove_rows_plural;
			}
	...
	tp.columns = {
		append: function( /* event */ ) {
			...
			if ( ! ( /^[1-9][0-9]{0,4}$/ ).test( num_columns ) ) {
				alert( iggogrid_strings.append_num_columns_invalid );
		...
		insert: function( event ) {
			...
			if ( 0 === $selected_columns.length ) {
				alert( iggogrid_strings.no_columns_selected );
				return;
			}
		...
		remove: function( /* event */ ) {
			...
			if ( tp.table.columns === $selected_columns.length ) {
				alert( iggogrid_strings.no_remove_all_columns );
				return;
			}

			if ( 1 === $selected_columns.length ) {
				confirm_message = iggogrid_strings.ays_remove_columns_singular;
			} else {
				confirm_message = iggogrid_strings.ays_remove_columns_plural;
			}
			if ( ! confirm( confirm_message ) ) {
				return;
			}
		...
		advanced_editor: {
			...
			buttonopen: function() {
				if ( ! tp.cells.advanced_editor.prompt_shown ) {
					if ( ! confirm( iggogrid_strings.advanced_editor_open ) ) {
						return;
					}
				}
	...
	tp.content = {
		link: {
			...
			add: function( /* event */ ) {
				if ( ! tp.content.link.prompt_shown ) {
					if ( ! confirm( iggogrid_strings.link_add ) ) {
						return;
					}
				}
		...
		image: {
			...
			add: function( /* event */ ) {
				if ( ! tp.content.image.prompt_shown ) {
					if ( ! confirm( iggogrid_strings.image_add ) ) {
		...
		span: {
			prompt_shown: false,
			add: function( span ) {
				var span_add_msg = ( '#rowspan#' === span ) ? iggogrid_strings.rowspan_add : iggogrid_strings.colspan_add;
				...
				if ( $id( 'option-use-datatables' ).prop( 'checked' ) ) {
					if ( confirm( iggogrid_strings.span_add_datatables_warning ) ) {
				...
				$id( 'edit-form-body' ).one( 'click', 'textarea', function() {
					...
					if ( '#rowspan#' === span ) {
						if ( 0 === row_idx ) {
							alert( iggogrid_strings.no_rowspan_first_row );
							return;
						}
						if ( tp.table.head && 1 === row_idx ) {
							alert( iggogrid_strings.no_rowspan_table_head );
							return;
						}
						if ( tp.table.foot && ( tp.table.rows - 1 ) === row_idx ) {
							alert( iggogrid_strings.no_rowspan_table_foot );
							return;
						}
					} else if ( ( '#colspan#' === span ) && ( tp.table.no_data_columns_pre === col_idx ) ) {
						alert( iggogrid_strings.no_colspan_first_col );
						return;
					}
					$textarea.val( span );
					tp.table.set_table_changed();
	...
	tp.check = {
		...
		changes_saved: function() {
			if ( tp.made_changes ) {
				return iggogrid_strings.unsaved_changes_unload;
			}
		}
	};
	...
	tp.save_changes = {
		trigger: function( event ) {
			...
			if ( event.shiftKey ) {
				...
				$id( 'iggogrid-page' ).find( 'form' ).submit();
				return;
			}

			$(this).closest( 'p' ).append( '<span class="animation-saving spinner" title="' + iggogrid_strings.saving_changes + '"/>' );
			$( '.save-changes-button' ).prop( 'disabled', true );
			$( 'body' ).addClass( 'wait' );
			//console.log(tp.table.prepare_ajax_request( 'iggogrid_save_table', '#nonce-edit-table' ));
			$.post(
					ajaxurl,
					tp.table.prepare_ajax_request( 'iggogrid_save_table', '#nonce-edit-table' ),
					function() { /* done with .success() below */ },
					'json'
				)
		...
		ajax_success: function( data, status /*, jqXHR */ ) {
			...
			if ( ( 'undefined' === typeof status ) || ( 'success' !== status ) ) {
				...
				if ( ( 'undefined' !== typeof data.error_details ) && ( iggogrid_options.print_debug_output ) ) {
		...
		success: function( data ) {
			...
			$id( 'edit-form' ).removeClass( 'iggogrid-edit-screen-id-' + tp.table.id ).addClass( 'iggogrid-edit-screen-id-' + data.table_id );
			...
			$( '.table-shortcode' ).val( '[' + iggogrid_options.shortcode + ' id=' + tp.table.new_id + ' /]' );
			// update the nonces
			...
			tp.save_changes.after_saving_dialog( 'success', iggogrid_strings[ data.message ] );
		},
		error: function( message ) {
		...
		after_saving_dialog: function( type, message ) {
			...
			$( '.animation-saving' ).closest( 'p' )
				.after( '<div class="ajax-alert ' + div_class + '"><p>' + iggogrid_strings['save_changes_' + type] + message + '</p></div>' );
	...
	tp.init = function() {
		...
		if ( iggogrid_options.cells_advanced_editor ) {
		...
		if ( iggogrid_options.cells_auto_grow ) {
		...
		$id( 'iggogrid-page' ).find( 'form' ).on( 'submit', function() {
			$(this).find( '.iggogrid-postbox-table' ).find( 'input, select' ).prop( 'disabled', false );
		} );
	...
	$(document).on('click','.iggogrid_get_column',function(e){
		...
		var current_show = jQuery.parseJSON($(".current_table_show").val());
		...
		$.post(ajaxurl, data, function(response) {
			if( response.success == true){
				var html = '<select multiple="multiple" size="2" class="iggo_grid_edit_columns" name="columns[]" style="width:200px;height:150px;">';
				$.each(response.message, function(i,val){
						if(current_show.contains(val)){
							html += '<option style="padding:3px 4px;" selected = "selected"  value ="'+val+'">'+val+'</option>';
						}else{
