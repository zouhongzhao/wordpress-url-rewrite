 ...
 * @since 1.0.0
 */
class IggoGrid_Editor_Button_Thickbox_View extends IggoGrid_View {
	 ...
	 * @since 1.0.0
	 *
	 * @var IggoGrid_Editor_Button_Thickbox_List_Table
	 */
	...
	public function setup( $action, array $data ) {
		...
		$this->wp_list_table = new IggoGrid_Editor_Button_Thickbox_List_Table();
...
?>
<title><?php printf( __( '%1$s &lsaquo; %2$s', 'iggogrid' ), __( 'List of Tables', 'iggogrid' ), 'IggoGrid' ); ?></title>
<style type="text/css">
...
#iggogrid-page .subtitle {
	float: left;
	padding: 10px 0 0;
}
#iggogrid-page .search-box input[name="s"] {
	float: left;
	width: auto;
}

/* Fix pagination layout */
#iggogrid-page .tablenav-pages {
	text-align: left;
}
#iggogrid-page .tablenav .tablenav-pages a {
	padding: 5px 12px;
	font-size: 16px;
}
#iggogrid-page .tablenav-pages .pagination-links .paging-input {
	font-size: 16px;
}

#iggogrid-page .tablenav-pages .pagination-links .current-page {
	padding: 4px;
	font-size: 16px;
}

/* Width and font weight for the columns */
.iggogrid-editor-button-list thead .column-table_id {
	width: 50px;
}
.iggogrid-editor-button-list tbody .column-table_id,
.iggogrid-editor-button-list tbody .column-table_name {
	font-weight: bold;
}
.iggogrid-editor-button-list thead .column-table_action {
	width: 150px;
}
.iggogrid-editor-button-list tbody .column-table_action {
	padding: 4px 7px 1px;
	vertical-align: middle;
}

/* Shortcode input field */
#iggogrid-page .table-shortcode-inline {
...
.rtl #iggogrid-page .search-box {
	float: left;
}
.rtl #iggogrid-page .subtitle {
	float: right;
}
.rtl #iggogrid-page .search-box input[name="s"] {
	float: right;
}
.rtl #iggogrid-page .table-shortcode-inline {
	width: 125px;
	font-size: 13px;
...
<h2><?php printf( __( '%1$s &lsaquo; %2$s', 'iggogrid' ), __( 'List of Tables', 'iggogrid' ), 'IggoGrid' ); ?></h2>
<div id="poststuff">
<p>
<?php _e( 'This is a list of all available tables.', 'iggogrid' ); ?> <?php _e( 'You may insert a table into a post or page here.', 'iggogrid' ); ?>
</p><p>
<?php printf( __( 'Click the &#8220;%1$s&#8221; button for the desired table to automatically insert the<br />corresponding Shortcode (%2$s) into the editor.', 'iggogrid' ), __( 'Insert Shortcode', 'iggogrid' ), '<input type="text" class="table-shortcode table-shortcode-inline ltr" value="' . esc_attr( '[' . IggoGrid::$shortcode . " id=<ID> /]" ) . '" readonly="readonly" />' ); ?>
</p>
<?php
	if ( ! empty( $_GET['s'] ) ) {
...
<form method="get" action="">
	<input type="hidden" name="action" value="iggogrid_<?php echo $this->action; ?>" />
	<?php wp_nonce_field( IggoGrid::nonce( $this->action ), '_wpnonce', false ); echo "\n"; ?>
	<?php $this->wp_list_table->search_box( __( 'Search Tables', 'iggogrid' ), 'tables_search' ); ?>
</form>
...
jQuery( document ).ready( function( $ ) {
	$( '.iggogrid-editor-button-list' ).on( 'click', '.insert-shortcode', function() {
...
} // class IggoGrid_Editor_Button_View


/**
 * IggoGrid Editor Button Thickbox List Table Class
 * @package IggoGrid
 * @subpackage Views
 * @author Tobias Bäthge
 ...
 * @uses WP_List_Table
 */
class IggoGrid_Editor_Button_Thickbox_List_Table extends WP_List_Table {
	...
	public function __construct() {
		parent::__construct( array(
			...
			'plural'	=> 'iggogrid-editor-button-list', // plural name of the listed records
	...
	public function get_columns() {
		$columns = array(
			...
			'table_action' => __( 'Action', 'iggogrid' ),
		);
		return $columns;
	}
	...
	protected function column_table_action( array $item ) {
		return '<input type="button" class="insert-shortcode button" title="' . esc_attr( '[' . IggoGrid::$shortcode . " id={$item['id']} /]" ) . '" value="' . esc_attr__( 'Insert Shortcode', 'iggogrid' ) . '" />';
	}
	...
	public function no_items() {
		...
		if ( 0 === $this->items_count ) {
			echo ' ' . __( 'You should add or import a table on the IggoGrid screens to get started!', 'iggogrid' );
		}
	}
	...
	protected function _search_callback( $item ) {
		...
		$item = IggoGrid::$model_table->load( $item, true, false ); // Load table again, with table data, but without options and visibility settings
	...
	public function prepare_items() {
		...
		foreach ( $this->items as &$item ) {
			$item = IggoGrid::$model_table->load( $item, false, false ); // Don't load data nor table options
		}
...
} // class IggoGrid_Editor_Button_Thickbox_List_Table
