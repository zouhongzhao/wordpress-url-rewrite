<?php
/**
 * Frontend Controller for IggoGrid with functionality for the frontend
 *
 * @package IggoGrid
 ...
 * @since 1.0.0
 */
class IggoGrid_Frontend_Controller extends IggoGrid_Controller {
	...
	public function __construct() {
		...
		/**
		 * Filter whether the IggoGrid Default CSS code shall be loaded.
		 *
		 * @since 1.0.0
		 *
		 * @param bool $use Whether the Default CSS shall be loaded. Default true.
		 */
		if ( apply_filters( 'iggogrid_use_default_css', true ) || IggoGrid::$model_options->get( 'use_custom_css' ) ) {
		...
		// Remove WP-Table Reloaded Shortcodes and CSS, and add IggoGrid Shortcodes
		...
		// make IggoGrid Shortcodes work in text widgets
		add_filter( 'widget_text', array( $this, 'widget_text_filter' ) );

		/**
		 * Filter whether the WordPress search shall also search IggoGrid tables.
		 *
		 * @since 1.0.0
		 *
		 * @param bool $search Whether the IggoGrid tables shall be searched. Default true.
		 */
		if ( apply_filters( 'iggogrid_wp_search_integration', true ) ) {
		...
		require_once IGGOGRID_ABSPATH . 'controllers/template-tag-functions.php';
	}

	/**
	 * Register IggoGrid Shortcodes, after removing WP-Table Reloaded Shortcodes
	 *
	 * @since 1.0.0
	 */
	public function init_shortcodes() {
		// Remove previously registered [table /] Shortcodes (e.g. from WP-Table Reloaded), as these would otherwise be used instead of IggoGrid's Shortcodes
		remove_shortcode( IggoGrid::$shortcode );
		remove_shortcode( IggoGrid::$shortcode_info );
		// Dequeue WP-Table Relaoded Default CSS, as it can influence IggoGrid table styling
		...
		add_shortcode( IggoGrid::$shortcode, array( $this, 'shortcode_table' ) );
		add_shortcode( IggoGrid::$shortcode_info, array( $this, 'shortcode_table_info' ) );
	}
	...
	public function enqueue_css() {
		...
		$use_default_css = apply_filters( 'iggogrid_use_default_css', true );
		$custom_css = IggoGrid::$model_options->get( 'custom_css' );
		$use_custom_css = ( IggoGrid::$model_options->get( 'use_custom_css' ) && '' != $custom_css );
		$use_custom_css_file = ( $use_custom_css && IggoGrid::$model_options->get( 'use_custom_css_file' ) );
		/**
		 ...
		 */
		$custom_css_version = apply_filters( 'iggogrid_custom_css_version', IggoGrid::$model_options->get( 'custom_css_version' ) );
		...
		$unfiltered_default_css_url = plugins_url( "css/default{$rtl}{$suffix}.css", IGGOGRID__FILE__ );
		/**
		 * Filter the URL from which the IggoGrid Default CSS file is loaded.
		 *
		 * @since 1.0.0
		 *
		 * @param string $unfiltered_default_css_url URL of the IggoGrid Default CSS file.
		 */
		$default_css_url = apply_filters( 'iggogrid_default_css_url', $unfiltered_default_css_url );

		$use_custom_css_combined_file = ( $use_default_css && $use_custom_css_file && $use_minified_css && ! is_rtl() && $unfiltered_default_css_url == $default_css_url && $iggogrid_css->load_custom_css_from_file( 'combined' ) );

		if ( $use_custom_css_combined_file ) {
			$custom_css_combined_url = $iggogrid_css->get_custom_css_location( 'combined', 'url' );
			// need to use 'iggogrid-default' instead of 'iggogrid-combined' to not break existing IggoGrid Extensions
			wp_enqueue_style( 'iggogrid-default', $custom_css_combined_url, array(), $custom_css_version );
		} else {
			$custom_css_dependencies = array();
			if ( $use_default_css ) {
				wp_enqueue_style( 'iggogrid-default', $default_css_url, array(), IggoGrid::version );
				$custom_css_dependencies[] = 'iggogrid-default'; // to make sure that Custom CSS is printed after Default CSS
			}

			$use_custom_css_minified_file = ( $use_custom_css_file && $use_minified_css && $iggogrid_css->load_custom_css_from_file( 'minified' ) );
			if ( $use_custom_css_minified_file ) {
				$custom_css_minified_url = $iggogrid_css->get_custom_css_location( 'minified', 'url' );
				wp_enqueue_style( 'iggogrid-custom', $custom_css_minified_url, $custom_css_dependencies, $custom_css_version );
				return;
			}

			$use_custom_css_normal_file = ( $use_custom_css_file && $iggogrid_css->load_custom_css_from_file( 'normal' ) );
			if ( $use_custom_css_normal_file ) {
				$custom_css_normal_url = $iggogrid_css->get_custom_css_location( 'normal', 'url' );
				wp_enqueue_style( 'iggogrid-custom', $custom_css_normal_url, $custom_css_dependencies, $custom_css_version );
				return;
			}

			if ( $use_custom_css ) {
				// get "Custom CSS" from options, try minified Custom CSS first
				$custom_css_minified = IggoGrid::$model_options->get( 'custom_css_minified' );
				 ...
				 */
				$custom_css = apply_filters( 'iggogrid_custom_css', $custom_css );
				if ( ! empty( $custom_css ) ) {
					// wp_add_inline_style() requires a loaded CSS file, so we have to work around that if "Default CSS" is disabled
					if ( $use_default_css ) {
						wp_add_inline_style( 'iggogrid-default', $custom_css ); // handle of the file to which the <style> shall be appended
					} else {
	...
	public function _print_custom_css() {
		// get "Custom CSS" from options, try minified Custom CSS first
		$custom_css = IggoGrid::$model_options->get( 'custom_css_minified' );
		if ( empty( $custom_css ) ) {
			$custom_css = IggoGrid::$model_options->get( 'custom_css' );
		}
		/** This filter is documented in controllers/controller-frontend.php */
	...
	protected function _enqueue_datatables() {
		 ...
		 */
		$js_url = apply_filters( 'iggogrid_datatables_js_url', $js_url, $js_file );
		wp_enqueue_script( 'jquery' );
		wp_enqueue_script( 'iggogrid-datatables', $js_url, array( 'jquery' ), IggoGrid::version, true );
	}
	...
	public function add_datatables_calls() {
		...
		foreach ( $this->shown_tables as $table_id => $table_store ) {
			...
			foreach ( $table_store['instances'] as $html_id => $js_options ) {
				 ...
				 */
				$datatables_locale = apply_filters( 'iggogrid_datatables_locale', $js_options['datatables_locale'], $table_id );
				// only do the expensive language file checks if they haven't been done yet
				if ( ! isset( $datatables_languages[ $datatables_locale ] ) ) {
					$orig_language_file = IGGOGRID_ABSPATH . "i18n/datatables/lang-{$datatables_locale}.json";
					/**
					 ...
					 */
					$language_file = apply_filters( 'iggogrid_datatables_language_file', $orig_language_file, $datatables_locale, IGGOGRID_ABSPATH ); // make sure to check file_exists( $new_file ) when using this filter!
				 ...
				 */
				$parameters = apply_filters( 'iggogrid_datatables_parameters', $parameters, $table_id, $html_id, $js_options );
				 ...
				 * @param array  $js_options The options for the JS library.
				 */
				$command = apply_filters( 'iggogrid_datatables_command', $command, $html_id, $parameters, $table_id, $js_options );
				if ( ! empty( $command ) ) {
					$commands[] = $command;
				}
			}
		 ...
		 */
		$commands = apply_filters( 'iggogrid_all_datatables_commands', $commands );
		if ( empty( $commands ) ) {
			return;
		}
...
JS;
	...
	public function shortcode_table( $shortcode_atts ) {
		 ...
		 */
		$default_shortcode_atts = apply_filters( 'iggogrid_shortcode_table_default_shortcode_atts', $default_shortcode_atts );
		...
		$shortcode_atts = apply_filters( 'iggogrid_shortcode_table_shortcode_atts', $shortcode_atts );
		
		// check, if a table with the given ID exists
		$table_id = preg_replace( '/[^a-zA-Z0-9_-]/', '', $shortcode_atts['id'] );
	
		if ( ! IggoGrid::$model_table->table_exists( $table_id ) ) {
			 ...
			 * @param string $table_id The current table ID.
			 */
			$message = apply_filters( 'iggogrid_table_not_found_message', $message, $table_id );
			return $message;
		}
		...
		if ( is_wp_error( $table ) ) {
			 ...
			 */
			$message = apply_filters( 'iggogrid_table_load_error_message', $message, $table_id, $table );
			return $message;
		}
		if ( isset( $table['is_corrupted'] ) && $table['is_corrupted'] ) {
			 ...
			 */
			$message = apply_filters( 'iggogrid_table_corrupted_message', $message, $table_id, $table['json_error'] );
			return $message;
		}
		 ...
		 */
		if ( ! is_null( $shortcode_atts['datatables_custom_commands'] ) && apply_filters( 'iggogrid_disable_custom_commands_shortcode_parameter', true ) ) {
		...
		$render_options['html_id'] = "iggogrid-{$table_id}";
		if ( $count > 1 ) {
		 ...
		 */
		$render_options['html_id'] = apply_filters( 'iggogrid_html_id', $render_options['html_id'], $table_id, $count );
		 ...
		 * @param string $table_id The current table ID.
		 */
		if ( is_user_logged_in() && apply_filters( 'iggogrid_edit_link_below_table', true, $table['id'] ) && current_user_can( 'iggogrid_edit_table', $table['id'] ) ) {
			$render_options['edit_table_url'] = IggoGrid::url( array( 'action' => 'edit', 'table_id' => $table['id'] ) );
		}
// 		echo "<pre>";
// 		print_r($render_options);
// 		echo "</pre>";
// 		die;
		/**
		 ...
		 */
		$render_options = apply_filters( 'iggogrid_table_render_options', $render_options, $table );
		...
		if ( $render_options['use_datatables'] && $render_options['table_head'] && count( $table['data'] ) > 1 ) {
			 ...
			 * @param array  $render_options The render options for the table.
			 */
			$js_options = apply_filters( 'iggogrid_table_js_options', $js_options, $table_id, $render_options );
		...
		if ( $render_options['cache_table_output'] && ! is_user_logged_in() ) {
			...
			$transient_name = 'iggogrid_' . $table_hash; // Attention: This string must not be longer than 45 characters!
			...
			if ( false === $output || '' == $output ) {
				...
				$caches_list_transient_name = 'iggogrid_c_' . md5( $table_id );
				 ...
				 */
				$output .= apply_filters( 'iggogrid_cache_hit_comment', "<!-- #{$render_options['html_id']} from cache -->" );
			}
		} else {
	...
	public function shortcode_table_info( $shortcode_atts ) {
		 ...
		 */
		$default_shortcode_atts = apply_filters( 'iggogrid_shortcode_table_info_default_shortcode_atts', $default_shortcode_atts );
		$shortcode_atts = shortcode_atts( $default_shortcode_atts, $shortcode_atts ); // Optional third argument left out on purpose. Use filter in the next line instead.
		/**
		 ...
		 */
		$shortcode_atts = apply_filters( 'iggogrid_shortcode_table_info_shortcode_atts', $shortcode_atts );
		 ...
		 * @param array       $shortcode_atts The attributes passed to the [table-info] Shortcode.
		 */
		$overwrite = apply_filters( 'iggogrid_shortcode_table_info_overwrite', false, $shortcode_atts );
		if ( $overwrite ) {
			return $overwrite;
		}
		...
		switch ( $field ) {
				...
				switch ( $format ) {
					...
					case 'human':
						...
						if ( $time_diff >= 0 && $time_diff < DAY_IN_SECONDS ) { // time difference is only shown up to one day
							$output = sprintf( __( '%s ago', 'iggogrid' ), human_time_diff( $modified_timestamp, $current_timestamp ) );
						} else {
							$output = IggoGrid::format_datetime( $table['last_modified'], 'mysql', '<br />' );
						}
						break 2;
					case 'mysql':
					default:
						$output = IggoGrid::format_datetime( $table['last_modified'], 'mysql', ' ' );
						break 2;
				}
				break;
			case 'last_editor':
				$output = IggoGrid::get_user_display_name( $table['options']['last_editor'] );
				break;
			case 'author':
				$output = IggoGrid::get_user_display_name( $table['author'] );
				break;
			case 'number_rows':
			...
			default:
				 ...
				 */
				$output = apply_filters( 'iggogrid_table_info_not_found_message', $output, $table, $field, $format );
		}
		 ...
		 * @param array  $shortcode_atts The attributes passed to the [table-info] Shortcode.
		 */
		$output = apply_filters( 'iggogrid_shortcode_table_info_output', $output, $table, $shortcode_atts );
		return $output;
	}
	 ...
	 *
	 * @param string $content Text content of the text widget, will be searched for one of IggoGrid's Shortcodes
	...
	public function widget_text_filter( $content ) {
		...
		// register IggoGrid's Shortcodes (which are then the only ones registered)
		add_shortcode( IggoGrid::$shortcode_info, array( $this, 'shortcode_table_info' ) );
		add_shortcode( IggoGrid::$shortcode, array( $this, 'shortcode_table' ) );
		// do the WP Shortcode routines on the widget text (i.e. search for IggoGrid's Shortcodes)
		$content = do_shortcode( $content );
		// restore the original Shortcodes (which includes IggoGrid's Shortcodes, for use in posts and pages)
	 ...
	 *
	 * This is done by looping through all search terms and IggoGrid tables and searching there for the search term,
	...
	public function posts_search_filter( $search_sql ) {
		...
		$table_ids = IggoGrid::$model_table->load_all( true, false ); // Load all table IDs and prime post meta cache for cached access to options and visibility settings of the tables, don't run filter hook
		...
		foreach ( $query_result as $search_term => $table_ids ) {
			...
			$regexp = '\\\\[' . IggoGrid::$shortcode . ' id=(["\\\']?)(' . $table_ids . ')([\]"\\\' /])'; // ' needs to be single escaped, [ double escaped (with \\) in mySQL
...
} // class IggoGrid_Frontend_Controller
